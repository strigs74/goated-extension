(function () {
  ...
  function captureRealValues() {
    ...
    if (nav && !realValues.has(nav) && /^[\d,]+$/.test(nav.textContent.trim())) {
      realValues.set(nav, nav.textContent);
    }
    document.querySelectorAll('span').forEach(span => {
      if (isBalanceSpan(span) && !realValues.has(span) && /^[\d,]+$/.test(span.textContent.trim())) {
  ...
  const balanceObserver = new MutationObserver(() => {
    // Always capture real values as they come in, even before we overwrite
    document.querySelectorAll('span').forEach(span => {
      if (isBalanceSpan(span) && /^[\d,]+$/.test(span.textContent.trim())) {
        const formatted = formatRobux(currentBalance);
        if (span.textContent.trim() !== formatted) {
          // This is a real value coming from Roblox — save it before overwriting
    ...
    const nav = document.getElementById('nav-robux-amount');
    if (nav && /^[\d,]+$/.test(nav.textContent.trim())) {
      const formatted = formatRobux(currentBalance);
      if (nav.textContent.trim() !== formatted) {
        if (!realValues.has(nav)) realValues.set(nav, nav.textContent);
        if (balanceEnabled) nav.textContent = formatted;
      }
    }
  });
